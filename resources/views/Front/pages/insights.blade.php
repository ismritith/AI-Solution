            ...
            <article class="lg:col-span-8 group cursor-pointer" data-aos="fade-right">
                <div class="glass-card rounded-3xl h-full flex flex-col p-5 space-y-6">
                    <div class="relative rounded-2xl overflow-hidden aspect-[16/9]">
                        <img class="w-full h-full object-cover transition-transform duration-700 group-hover:scale-103 grayscale group-hover:grayscale-0" src="{{ asset($featuredPost->banner_image) }}" alt="{{ $featuredPost->title }}"/>
                        <div class="absolute top-4 left-4 flex gap-2">
                            <span class="bg-secondary/20 backdrop-blur-md border border-secondary/30 text-secondary font-mono text-[10px] font-bold px-3 py-1 rounded-full uppercase">{{ $featuredPost->category }}</span>
                            <span class="bg-primary/60 backdrop-blur-md border border-white/10 text-white font-mono text-[10px] px-3 py-1 rounded-full">{{ $featuredPost->reading_time }} min read</span>
                        </div>
                    </div>
                    ...
                    <div class="space-y-4 px-2 pb-2 flex-1 flex flex-col justify-between">
                        <div>
                            <h2 class="font-display text-2xl md:text-3xl font-extrabold text-white group-hover:text-secondary transition-colors leading-tight">
                                <a href="/insights1?id={{ $featuredPost->id }}" class="hover:underline">{{ $featuredPost->title }}</a>
                            </h2>
                        ...
                        <div class="flex items-center justify-between pt-4 border-t border-white/5 mt-4">
                            <div class="flex items-center gap-3">
                                ...
                                <div>
                                    <p class="font-display text-sm font-bold text-white">{{ $featuredPost->author_name }}</p>
                                    <p class="font-mono text-[10px] text-on-surface-variant uppercase tracking-widest font-bold">{{ $featuredPost->author_role }}</p>
                                </div>
                            </div>
                            <span class="text-xs text-on-surface-variant font-label-mono">{{ $featuredPost->published_at ? $featuredPost->published_at->format('M d, Y') : '' }}</span>
                        </div>
                    </div>
                </div>
            </article>
        ...
        <div class="lg:col-span-4 flex flex-col gap-8" data-aos="fade-left" data-aos-delay="200">
            @forelse($remainingPosts->take(2) as $sidePost)
                <article class="glass-card rounded-3xl p-8 group cursor-pointer flex flex-col justify-between h-full">
                    <div class="space-y-4">
                        <span class="bg-accent/15 border border-accent/30 text-accent font-mono text-[10px] px-2.5 py-0.5 rounded-full font-bold uppercase inline-block">{{ $sidePost->category }}</span>
                    ...
                    <div class="flex items-center justify-between pt-6 border-t border-white/5 mt-6">
                        <span class="font-mono text-[10px] text-on-surface-variant uppercase tracking-widest font-bold">{{ $sidePost->published_at ? $sidePost->published_at->format('M d, Y') : '' }}</span>
                        <a href="/insights1?id={{ $sidePost->id }}" class="material-symbols-outlined text-secondary group-hover:translate-x-1 transition-transform">
                            arrow_forward
                        </a>
                    </div>
                </article>
    ...
    @if($remainingPosts->count() > 2)
        <div class="grid grid-cols-1 md:grid-cols-3 gap-8">
            @foreach($remainingPosts->skip(2) as $post)
                <article class="glass-card rounded-3xl overflow-hidden group cursor-pointer flex flex-col justify-between" data-aos="fade-up">
                    <div>
                        <div class="aspect-video relative overflow-hidden">
                            <img class="w-full h-full object-cover grayscale group-hover:grayscale-0 transition-all duration-500" src="{{ asset($post->banner_image) }}" alt="{{ $post->title }}"/>
                        </div>
                        <div class="p-8 space-y-4">
                            <div class="flex gap-3">
                                <span class="font-mono text-[10px] text-secondary font-bold uppercase tracking-wider">{{ $post->category }}</span>
                                <span class="font-mono text-[10px] text-on-surface-variant">{{ $post->reading_time }} MIN READ</span>
                            </div>
                    ...
                    <div class="px-8 pb-8 pt-2">
                        <a href="/insights1?id={{ $post->id }}" class="flex items-center gap-2 text-secondary font-body text-xs font-bold hover:text-white transition-colors">
...
<div id="insightModal" class="fixed inset-0 z-[100] hidden items-center justify-center p-4 md:p-10 transition-all duration-300">
    <!-- Modal Backdrop -->
    <div id="modalBackdrop" class="absolute inset-0 bg-[#05020c]/85 backdrop-blur-xl opacity-0 transition-opacity duration-300"></div>
    ...
    <div id="modalContent" class="relative w-full max-w-4xl max-h-[85vh] bg-[#0d071d]/90 border border-white/10 rounded-[32px] overflow-hidden shadow-2xl flex flex-col transform scale-95 opacity-0 transition-all duration-300 z-10">
        ...
        <div id="modalBody" class="flex-1 overflow-y-auto p-6 md:p-12 custom-scrollbar">
            ...
            <div id="modalSkeleton" class="space-y-8 animate-pulse">
                ...
                <div class="h-20 bg-white/5 rounded-xl w-full text-on-surface/50 text-xs flex items-center justify-center font-mono">RETRIEVING NEURAL ARCHIVE NODE...</div>
