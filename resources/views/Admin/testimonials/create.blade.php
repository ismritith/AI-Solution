...
@section('title', 'AI-Solutions Admin | Log Feedback')

@section('content')
<div class="glass-card rounded-2xl p-6 max-w-3xl mx-auto">
    <div class="mb-6">
        <h3 class="text-on-surface font-semibold text-lg">Log Customer Feedback</h3>
        <p class="text-xs text-on-surface-variant">Inject a new customer feedback review node</p>
    </div>
    ...
    <form action="{{ route('admin.testimonials.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
                ...
                <input type="text" name="client_name" required placeholder="e.g. Marcus Vance" class="w-full bg-surface-container border border-white/10 rounded-xl px-4 py-2.5 text-sm text-on-surface placeholder:text-on-surface-variant/30 focus:ring-primary focus:border-primary focus:ring-1">
            </div>
            ...
            <div>
                ...
                <input type="text" name="client_role" required placeholder="e.g. CTO, Neural Networks Corp" class="w-full bg-surface-container border border-white/10 rounded-xl px-4 py-2.5 text-sm text-on-surface placeholder:text-on-surface-variant/30 focus:ring-primary focus:border-primary focus:ring-1">
            </div>
        </div>

        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
            <div>
                <label class="block text-xs font-label-mono uppercase tracking-widest text-on-surface-variant mb-2">Client Avatar Image (Optional)</label>
            ...
            <div class="flex items-center mt-6">
                <label class="inline-flex items-center cursor-pointer">
                    <input type="checkbox" name="is_verified" class="sr-only peer">
        ...
        <div>
            ...
            <input type="hidden" name="quote_text" id="quote_text_input">
            <div id="quote_text_editor" class="min-h-[150px]"></div>
        </div>
        ...
        <div class="flex justify-end gap-4 pt-4 border-t border-white/10">
            ...
            <button type="submit" class="py-2.5 px-5 rounded-xl bg-gradient-to-r from-primary-container to-secondary-container text-white font-semibold text-sm hover:shadow-[0_0_20px_rgba(124,58,237,0.4)] transition-all">Submit Feedback</button>
        </div>
    </form>
</div>
@endsection
