    ...
    <form action="{{ route('admin.gallery.update', $asset) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <div class="grid grid-cols-2 gap-3 p-1 rounded-xl bg-white/5 border border-white/5">
                    <button type="button" id="media_type_image" class="py-2 rounded-lg text-sm font-semibold flex items-center justify-center gap-1.5 transition-all {{ $asset->media_type === 'image' ? 'bg-secondary text-white shadow-lg shadow-secondary/20' : 'text-on-surface-variant hover:bg-white/5' }}" onclick="setMediaType('image')">
                        <span class="material-symbols-outlined text-[18px]">image</span> Image
                    </button>
                    <button type="button" id="media_type_video" class="py-2 rounded-lg text-sm font-semibold flex items-center justify-center gap-1.5 transition-all {{ $asset->media_type === 'video' ? 'bg-secondary text-white shadow-lg shadow-secondary/20' : 'text-on-surface-variant hover:bg-white/5' }}" onclick="setMediaType('video')">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
                ...
                <div class="grid grid-cols-2 gap-3 p-1 rounded-xl bg-white/5 border border-white/5">
                    <button type="button" id="upload_method_file" class="py-2 rounded-lg text-sm font-semibold flex items-center justify-center gap-1.5 transition-all {{ $asset->upload_method === 'upload' ? 'bg-secondary text-white shadow-lg shadow-secondary/20' : 'text-on-surface-variant hover:bg-white/5' }}" onclick="setUploadMethod('upload')">
                        <span class="material-symbols-outlined text-[18px]">cloud_upload</span> Upload File
                    </button>
                    <button type="button" id="upload_method_link" class="py-2 rounded-lg text-sm font-semibold flex items-center justify-center gap-1.5 transition-all {{ $asset->upload_method === 'link' ? 'bg-secondary text-white shadow-lg shadow-secondary/20' : 'text-on-surface-variant hover:bg-white/5' }}" onclick="setUploadMethod('link')">
        ...
        <div class="space-y-4">
            ...
            <div id="file_container" class="{{ $asset->upload_method === 'upload' ? '' : 'hidden' }} space-y-2">
                <label id="file_label" class="block text-xs font-label-mono uppercase tracking-widest text-on-surface-variant mb-2">{{ $asset->media_type === 'video' ? 'Replace Video File (Optional)' : 'Replace Image File (Optional)' }}</label>
            ...
            <div id="url_container" class="{{ $asset->upload_method === 'link' ? '' : 'hidden' }} space-y-2">
                <label id="url_label" class="block text-xs font-label-mono uppercase tracking-widest text-on-surface-variant mb-2">{{ $asset->media_type === 'video' ? 'YouTube Video Embed URL' : 'External Image URL' }}</label>
                <input type="url" name="external_url" value="{{ $asset->external_url }}" placeholder="{{ $asset->media_type === 'video' ? 'https://www.youtube.com/embed/...' : 'https://images.unsplash.com/...' }}" class="w-full bg-surface-container border border-white/10 rounded-xl px-4 py-2.5 text-sm text-on-surface placeholder:text-on-surface-variant/30 focus:ring-primary focus:border-primary focus:ring-1">
                <p id="url_help" class="text-[10px] text-on-surface-variant">{{ $asset->media_type === 'video' ? 'Provide a valid YouTube embed link (e.g., https://www.youtube.com/embed/dQw4w9WgXcQ)' : 'Provide the direct image URL.' }}</p>
            </div>
...
<script>
    function setMediaType(type) {
        ...
        valInput.value = type;
        if (type === 'image') {
            imgBtn.className = 'py-2 rounded-lg text-sm font-semibold flex items-center justify-center gap-1.5 transition-all bg-secondary text-white shadow-lg shadow-secondary/20';
            vidBtn.className = 'py-2 rounded-lg text-sm font-semibold flex items-center justify-center gap-1.5 transition-all text-on-surface-variant hover:bg-white/5';
            ...
            vidBtn.className = 'py-2 rounded-lg text-sm font-semibold flex items-center justify-center gap-1.5 transition-all bg-secondary text-white shadow-lg shadow-secondary/20';
            imgBtn.className = 'py-2 rounded-lg text-sm font-semibold flex items-center justify-center gap-1.5 transition-all text-on-surface-variant hover:bg-white/5';
            fileLabel.textContent = 'Replace Video File (Optional)';
            urlLabel.textContent = 'YouTube Video Embed URL';
            urlHelp.textContent = 'Provide a valid YouTube embed link (e.g., https://www.youtube.com/embed/dQw4w9WgXcQ)';
    ...
    function setUploadMethod(method) {
        ...
        if (method === 'upload') {
            fileBtn.className = 'py-2 rounded-lg text-sm font-semibold flex items-center justify-center gap-1.5 transition-all bg-secondary text-white shadow-lg shadow-secondary/20';
            linkBtn.className = 'py-2 rounded-lg text-sm font-semibold flex items-center justify-center gap-1.5 transition-all text-on-surface-variant hover:bg-white/5';
            fileContainer.classList.remove('hidden');
            urlContainer.classList.add('hidden');
        } else {
            linkBtn.className = 'py-2 rounded-lg text-sm font-semibold flex items-center justify-center gap-1.5 transition-all bg-secondary text-white shadow-lg shadow-secondary/20';
            fileBtn.className = 'py-2 rounded-lg text-sm font-semibold flex items-center justify-center gap-1.5 transition-all text-on-surface-variant hover:bg-white/5';
