    ...
    <form action="{{ route('admin.gallery.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <div class="grid grid-cols-2 gap-3 p-1 rounded-xl bg-white/5 border border-white/5">
                    <button type="button" id="media_type_image" class="py-2 rounded-lg text-sm font-semibold flex items-center justify-center gap-1.5 transition-all bg-secondary/80 text-white shadow-lg shadow-secondary/20" onclick="setMediaType('image')">
                        <span class="material-symbols-outlined text-[18px]">image</span> Image
                    </button>
                    <button type="button" id="media_type_video" class="py-2 rounded-lg text-sm font-semibold flex items-center justify-center gap-1.5 transition-all text-on-surface-variant hover:bg-white/5" onclick="setMediaType('video')">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
                ...
                <div class="grid grid-cols-2 gap-3 p-1 rounded-xl bg-white/5 border border-white/5">
                    <button type="button" id="upload_method_file" class="py-2 rounded-lg text-sm font-semibold flex items-center justify-center gap-1.5 transition-all bg-secondary/80 text-white shadow-lg shadow-secondary/20" onclick="setUploadMethod('upload')">
                        <span class="material-symbols-outlined text-[18px]">cloud_upload</span> Upload File
                    </button>
                    <button type="button" id="upload_method_link" class="py-2 rounded-lg text-sm font-semibold flex items-center justify-center gap-1.5 transition-all text-on-surface-variant hover:bg-white/5" onclick="setUploadMethod('link')">
        ...
        <div class="space-y-4">
            ...
            <div id="file_container" class="space-y-2">
                <label id="file_label" class="block text-xs font-label-mono uppercase tracking-widest text-on-surface-variant mb-2">Upload Image File</label>
            ...
            <div id="url_container" class="hidden space-y-2">
                <label id="url_label" class="block text-xs font-label-mono uppercase tracking-widest text-on-surface-variant mb-2">External URL</label>
                <input type="url" name="external_url" placeholder="https://www.youtube.com/embed/..." class="w-full bg-surface-container border border-white/10 rounded-xl px-4 py-2.5 text-sm text-on-surface placeholder:text-on-surface-variant/30 focus:ring-primary focus:border-primary focus:ring-1">
